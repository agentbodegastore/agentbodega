...
function positiveIntegerArg(args, name, fallback, min, max) {
...
function snippets(endpoint) {
    ...
    const curl = `curl -sS -X ${method} ${shellQuote(String(endpoint.url || ""))} \\\n` +
...
const toolDefinitions = [
        ...
        inputSchema: {
            ...
            properties: {
                task: {
                    ...
                    minLength: 1,
                    maxLength: 2000,
                    description: "Plain-language task to solve with AgentBodega routes, for example: check whether my deploy failure is caused by GitHub, npm, Vercel, Supabase, or OpenAI."
                },
                budgetUsd: {
                    type: "number",
                    minimum: 0,
    ...
    },
    {
        ...
        description: "Use this first when you need to choose an AgentBodega paid data endpoint. It fetches the live directory and returns matching endpoint summaries with key, path, price, category, tags, required inputs, and example request. It never executes paid endpoints or creates x402 payments.",
        inputSchema: {
            type: "object",
            additionalProperties: false,
            properties: {
...
async function callTool(name, args) {
    if (name === "agentbodega_plan_task") {
        ...
        const planned = await postJson("/api/plan", { task, budgetUsd, prefer, limit });
    ...
    if (name === "agentbodega_payment_guide") {
        ...
        return toolJson({
            ...
            savings: {
                ...
                policy: "Agents may present AgentBodega savings offers, including social or referral-style promos, only as optional choices. Do not post, reply, follow, like, subscribe, tag, hashtag, DM, or join a list unless the owner approves the exact action, account, and public text.",
                preferredActions: ["disclosed share", "referral", "MCP install proof", "directory submission", "benchmark or result screenshot"],
                skipWhen: "Skip offers that require bulk, fake, repeated, deceptive, or platform-policy-breaking engagement. Never ask for social credentials."
            },
            curl: includeCurl
                ? {
...
async function readResource(uri) {
    ...
    if (uri === "agentbodega://l402") {
        const current = await fetchJson("/.well-known/l402-services");
        return { contents: [{ uri, mimeType: "application/json", text: jsonText(current) }] };
    }
...
function getPrompt(name, args) {
    if (name !== "agentbodega_start")
        throw new RpcError(-32601, `Unknown prompt: ${name}`);
